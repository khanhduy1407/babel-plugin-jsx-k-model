      ...
      JSXOpeningElement: function JSXOpeningElement(path) {
        ...
        path.get('attributes').forEach(function(path) {
          ...
          if (t.isJSXIdentifier(path.node.name)) {
            if (path.node.name.name !== 'k-model') {
              return
            }
          } else if (t.isJSXNamespacedName(path.node.name)) {
            if (path.node.name.namespace.name !== 'k-model') {
              return
            }
            ...
            if (!~allowedModifiers.indexOf(path.node.name.name.name)) {
              throw path
                .get('name')
                .get('name')
                .buildCodeFrameError('k-model does not support "' + path.node.name.name.name + '" modifier')
            }
          ...
          if (model) {
            throw path.buildCodeFrameError('you can not have multiple k-model directives on a single element')
          }

          if (!t.isJSXExpressionContainer(path.node.value)) {
            throw path.get('value').buildCodeFrameError('you should use JSX Expression with k-model')
          }
          ...
          if (!t.isMemberExpression(path.node.value.expression)) {
            throw path
              .get('value')
              .get('expression')
              .buildCodeFrameError('you should use MemberExpression with k-model')
          }
        ...
        if (tag === 'input' && typePath && t.isJSXExpressionContainer(typePath.node)) {
          throw typePath.buildCodeFrameError('you can not use dynamic type with k-model')
        }
        if (tag === 'input' && type === 'file') {
          throw typePath.buildCodeFrameError('you can not use "file" type with k-model')
        }
        ...
        if (tag === 'select') {
          ...
          throw path.buildCodeFrameError('you can not use "' + tag + '" with k-model')
        }
